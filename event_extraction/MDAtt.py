...
def psan(opi_rep_tensor,ent_rep_tensor,rep_mask,num_hiddens=100,keep_prob=1,is_train=None,initializer=None):
    # print(op_rep_tensor)
    with tf.variable_scope('psan'):
        with tf.variable_scope('op_att_ta'):
            new_opi_rep,opi_att_scores = normal_attention(opi_rep_tensor,ent_rep_tensor,rep_mask,initializer=initializer) #bl,sl,vec
        with tf.variable_scope('ta_att_op'):
            new_ent_rep,ent_att_scores = normal_attention(ent_rep_tensor,opi_rep_tensor,rep_mask,initializer=initializer)#bl,sl,vec
    # print(new_ta_rep)
...
def normal_attention(query_rep,value_rep,rep_mask,keep_prob=1.0,is_train=None,initializer = None):
    bs, sl, vec = tf.shape(query_rep)[0], tf.shape(query_rep)[1], tf.shape(query_rep)[2]
    ivec = query_rep.get_shape()[-1]
    value_rep_tile = tf.tile(tf.expand_dims(value_rep, 1), [1, sl, 1, 1])  # bs,sl,sl,vec
